document.addEventListener('DOMContentLoaded', async () => {
    ...
    function nextPage() {
        ...
        if (currentPage < totalPages) {
            currentPage++;
            renderCurrentPage();
            if (blogFeed) blogFeed.scrollIntoView({ behavior: 'smooth', block: 'start' });
        }
    }
    ...
    function renderCategories() {
        if (!categoryList || !mobileCategoryListCustom) return;
    ...
    function renderTags() {
        if (!tagContainer || !mobileTagList || !mobileTagListLabel || !tagList) return;
    ...
    function showSnackbar(message) {
        if (!snackbarMessage || !snackbar) return;
    ...
    function updateCategoryUI() {
        ...
        if (mobileCategoryLabel) mobileCategoryLabel.textContent = selectedText;

        if (mobileCategoryListCustom) {
    ...
    function showPost(postId) {
        ...
        // --- Basic Post Info ---
        if (document.getElementById('post-title')) document.getElementById('post-title').innerHTML = `<i data-lucide="book-open" class="w-10 h-10 -mt-2 mr-3 inline-block text-gray-500"></i> ${post.title}`;
        if (document.getElementById('post-date')) document.getElementById('post-date').innerHTML = `<i data-lucide="calendar-days" class="w-4 h-4 mr-2 inline-block text-gray-500"></i> ${post.date}`;
        ...
        if (postContentEl) {
            ...
            if (currentTheme === 'dark' || currentTheme === 'slate') {
                postContentEl.classList.add('prose-invert');
            } else {
                postContentEl.classList.remove('prose-invert');
            }
            postContentEl.innerHTML = post.fullContent;
            postContentEl.classList.add('prose-headings:font-bold');
        }
    ...
    function closeMobileMenu() {
        if (mobileMenu && !mobileMenu.classList.contains('hidden')) {
            ...
            if (menuIconClosed) menuIconClosed.classList.remove('hidden');
    ...
    if (mobileCategoryToggle) {
        mobileCategoryToggle.addEventListener('click', () => {
            if (mobileCategoryMenu) mobileCategoryMenu.classList.toggle('hidden');
            if (mobileCategoryChevron) {
    ...
    if (mobileCategoryMenu) {
        mobileCategoryMenu.addEventListener('click', (e) => {
            ...
            if (link) {
                ...
                if (mobileCategoryChevron) mobileCategoryChevron.style.transform = 'rotate(0deg)';
                window.location.hash = ''; // <-- Set hash to navigate
            }
        });
    }
    ...
    window.addEventListener('click', (e) => {
        if (mobileCategoryToggle && !mobileCategoryToggle.contains(e.target) && 
            mobileCategoryMenu && !mobileCategoryMenu.contains(e.target)) {
    ...
    if (tagList) tagList.addEventListener('click', handleTagClick);
    if (mobileTagList) mobileTagList.addEventListener('click', handleTagClick);
    ...
    if (mobileMenuButton) {
        mobileMenuButton.addEventListener('click', () => {
            ...
            if (menuIconClosed) menuIconClosed.classList.toggle('hidden');
    ...
    async function handleLikePost(postId) {
        ...
        try {
            ...
            if (likeCount) likeCount.textContent = newCount - 1; // Revert count
